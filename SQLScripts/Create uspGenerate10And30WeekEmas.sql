...
DECLARE @Periods1 int, @Periods5 int, @Periods10 int, @Periods30 int, @Periods8d int, @K1 FLOAT, @K5 FLOAT, @K10 FLOAT, @K30 FLOAT, @K8d FLOAT
DECLARE @Ema1 FLOAT, @Ema5 FLOAT, @Ema10 FLOAT, @Ema30 FLOAT, @Ema8d FLOAT
SET @Periods1 = 5;
SET @Periods5 = 25;
SET @Periods10 = 50;
SET @Periods30 = 150;
...
UPDATE #TBL_EMA_RT
	...
	,	[StartingAverage8d] = t8d.[StartAvg]
FROM #TBL_EMA_RT rt
...
JOIN #TBL_START_AVG_8d t8d ON t8d.[ShareId] = rt.[ShareId]

DROP TABLE #TBL_START_AVG_1
DROP TABLE #TBL_START_AVG_5
DROP TABLE #TBL_START_AVG_10
DROP TABLE #TBL_START_AVG_30
DROP TABLE #TBL_START_AVG_8d
...
UPDATE #TBL_EMA_RT
	...
	,	@Ema8d =
			CASE
			WHEN [Day#] < @Periods8d THEN NULL
            WHEN [Day#] = @Periods8d THEN rt.[AdjustedClose] * @K8d + rt.[StartingAverage8d] * (1 -@K8d)
            WHEN [Day#] > @Periods8d THEN rt.[AdjustedClose] * @K8d + @Ema8d * (1 - @K8d)
			END
	,	Ema1 = @Ema1 
	,	Ema5 = @Ema5
	,	Ema10 = @Ema10 
...
UPDATE [ShareIndicators]
	...
	,	[ClosedAboveEma8d] = IIF(q.[AdjustedClose] >= rt.[Ema8d], 1, 0)
...
INSERT INTO [dbo].[ShareIndicators] ([Id], [ShareId], [Day], [Ema1], [Ema5], [Ema10], [Ema30], [Ema8d], [ClosedAboveEma10], [ClosedAboveEma30], [WeeklyMomentum], [ClosedAboveEma8d])
	SELECT NEWID() AS Id
		,	rt.[ShareId]
		,	rt.[Day]
		,	rt.[Ema1]
		...
		,	rt.[Ema1]-rt.[Ema5]
		,	IIF(q.[AdjustedClose] >= rt.[Ema8d], 1, 0)
	FROM #TBL_EMA_RT rt
